...
function bakeParticleAnimation(settings: ParticleSettings): BakedFrame[] {
  ...
  // For looping animations, add a duplicate of the first frame at the end
  // to ensure seamless looping in Spine
  if (settings.emitter.looping && frames.length > 0) {
    const firstFrame = frames[0];
    const lastFrameTime = (frameCount) * dt;
    const loopFrame: BakedFrame = {
      time: lastFrameTime,
      particles: new Map(firstFrame.particles)
    };
...
const ParticleSpineExporter: React.FC = () => {
  ...
  }, [isPlaying, showEmitter, zoom, spriteCanvas, backgroundImage, bgPosition]);
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v78
          </h1>
          <p className="text-xs text-slate-400">New: Loop Mode • Prewarm • Start Delay • Seamless Looping</p>
        </header>
