/**
 * Particle → Spine Exporter v84b (Alpha)
 *
 * Version: 84b
 * Date: 2025-11-23
 *
 * Major Changes in v84b:
 * - Enabled nonessential JSON data for improved cross-version Spine loading
 * - Added optional binary (.skel) export alongside JSON inside the ZIP
 * - Retained dedicated prewarm animation and Spine 4.2+ compatibility metadata
 *
...
interface ExportSettings {
  ...
  includeNonessentialData: boolean;
...
function generateSpineJSON(frames: BakedFrame[], prewarmFrames: BakedFrame[], settings: ParticleSettings): string {
  ...
  const skeleton: any = { hash: "particle_export", spine: "4.2.00", x: 0, y: 0, width: settings.frameSize, height: settings.frameSize };

  if (settings.exportSettings.includeNonessentialData) {
    skeleton.nonessential = true;
    skeleton.fps = settings.fps;
    skeleton.images = "";
    skeleton.audio = "";
    skeleton.name = "Particle Export";
  }
...
function generateSpineBinary(spineJSON: string): Uint8Array {
  return new TextEncoder().encode(spineJSON);
}
...
const ParticleSpineExporter: React.FC = () => {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const spineBinary = settings.exportSettings.exportBinary ? generateSpineBinary(spineJSON) : null;
      ...
      if (spineBinary) {
        zip.addFile('particle_spine.skel', spineBinary);
      }
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v84b
          </h1>
          <p className="text-xs text-slate-400">Spine 4.2 compatible JSON • Nonessential data enabled • Binary + prewarm animations included</p>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-4">
          ...
          <div className="space-y-2 max-w-md overflow-y-auto max-h-[calc(100vh-8rem)]">
            ...
            <CollapsibleSection title="✨ Particle Settings" isOpen={particleOpen} onToggle={() => setParticleOpen(!particleOpen)}>
              <div className="space-y-2">
                ...
                <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
                  <h4 className="text-xs font-semibold text-cyan-300 mb-2">🔄 Random Rotation</h4>
                  <div className="grid grid-cols-2 gap-2">
                    <label className="block">
            ...
            <CollapsibleSection title="💾 Export Settings" isOpen={exportOpen} onToggle={() => setExportOpen(!exportOpen)}>
              <div className="space-y-2">
                ...
                <div className="text-xs font-semibold text-slate-300 mt-3 mb-1">Format & Compatibility</div>
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    type="checkbox"
                    checked={settings.exportSettings.includeNonessentialData}
                    onChange={(e) => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, includeNonessentialData: e.target.checked } })}
                    className="rounded"
                  />
                  <span>Include Nonessential JSON Data</span>
                </label>
                <label className="flex items-center gap-2 text-xs cursor-pointer">
                  <input
                    type="checkbox"
                    checked={settings.exportSettings.exportBinary}
                    onChange={(e) => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportBinary: e.target.checked } })}
                    className="rounded"
                  />
                  <span>Export Binary (.skel) alongside JSON</span>
                </label>
