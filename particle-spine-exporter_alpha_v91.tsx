/**
 * Particle → Spine Exporter v91 (Alpha)
 *
 * Version: 91
 * Date: 2025-11-25
 *
 * Major Changes in v91:
 * - Fixed prewarm animation data recording: visible particles from last frame of loop animation
 *   are now correctly copied into prewarm animation to ensure smooth looping transitions
 */
...
function generateSpineJSON(frames: BakedFrame[], prewarmFrames: BakedFrame[], settings: ParticleSettings): string {
  ...
  if (prewarmAnimation && loopAnimation && frames.length > 0 && prewarmFrames.length > 0) {
    ...
    for (const particleId of visibleParticleIds) {
      ...
      // Copy bone animations (translate, rotate, scale)
      if (loopAnimation.bones[boneName]) {
        if (!prewarmAnimation.bones[boneName]) {
          prewarmAnimation.bones[boneName] = {};
        }
      ...
      // Copy slot animations (attachment, rgba)
      if (loopAnimation.slots[slotName]) {
        if (!prewarmAnimation.slots[slotName]) {
          prewarmAnimation.slots[slotName] = {};
        }
