/**
 * Particle → Spine Exporter v85 (Alpha)
*
 * Version: 85
 * Date: 2025-11-23
*
 * Major Changes in v85:
 * - Added header-only drag handles and three-column layout for panel docking
 * - Normalized lifetime curves (0–1) layered over randomized min/max ranges
 * - Moved vortex toggle into its force tab and restored checkbox controls for loop/prewarm
*
...
const DEFAULT_SETTINGS: ParticleSettings = {
  ...
  vortexStrengthMin: -500,
  vortexStrengthMax: 500,
  ...
  update(dt: number, skipTimeReset: boolean = false) {
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      if (this.settings.nonUniformScale) {
        p.scaleX = sizeValue * this.settings.scaleRatioX;
        p.scaleY = sizeValue * this.settings.scaleRatioY;
      } else {
        p.scaleX = sizeValue;
        p.scaleY = sizeValue;
      }
      ...
      const weightMultiplier = lerp(
        this.settings.weightMin,
        this.settings.weightMax,
        clamp01(evaluateCurve(this.settings.weightOverLifetime, t))
      );
...
const ParticleSpineExporter: React.FC = () => {
  ...
  const PANEL_IDS = ['emitter', 'particle', 'color', 'forces', 'curves', 'export'] as const;
  ...
  const panelLookup: Record<string, JSX.Element> = {
            ...
            <label className="block">
              ...
              <div className="flex items-center gap-2 mt-1">
                <input
                  ...
                  onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, prewarm: e.target.checked }})}
                  className="accent-purple-500"
                />
                <span className="text-xs text-slate-200">Enable</span>
              </div>
            </label>
          </div>
            ...
            <label className="block">
              ...
              <div className="flex items-center gap-2 mt-1">
                <input
                  ...
                  onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, looping: e.target.checked }})}
            ...
            <div className="space-y-2 pl-2 border-l-2 border-blue-500">
              <div className="grid grid-cols-2 gap-2">
                ...
                <label className="block">
                  ...
                  <input
                    type="number"
                    min={settings.emitter.durationStart}
                    max={settings.duration}
                    step="0.1"
    ...
    color: (
      <CollapsibleSection title="🎨 Color Over Lifetime" isOpen={colorOpen} onToggle={() => setColorOpen(!colorOpen)}>
        <ColorGradientEditor
          ...
          onReset={() => updateSettings({
            ...settings,
            colorOverLifetime: {
              points: DEFAULT_SETTINGS.colorOverLifetime.points.map(p => ({
                time: p.time,
                color: { ...p.color }
              }))
            }
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setForceTab(tab.id as any)}
                className={`px-2 py-1 rounded text-[11px] border ${forceTab === tab.id ? 'bg-purple-600 border-purple-400' : 'bg-slate-800 border-slate-700 hover:bg-slate-700'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          {forceTab === 'gravity' && (
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.gravityMin}
                  onChange={e => updateSettings({ ...settings, gravityMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.gravityMax}
                  onChange={e => updateSettings({ ...settings, gravityMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  step="0.01"
                  value={settings.dragMin}
                  onChange={e => updateSettings({ ...settings, dragMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  step="0.01"
                  value={settings.dragMax}
                  onChange={e => updateSettings({ ...settings, dragMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
          ...
          {forceTab === 'noise' && (
            <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
            <h4 className="text-xs font-semibold text-amber-300 mb-2">🌪️ Noise</h4>
            <div className="grid grid-cols-2 gap-2 mb-2">
              <label className="block">
                <span className="text-xs text-slate-300">Strength Min</span>
                <input
                  type="number"
                  value={settings.noiseStrengthMin}
                  onChange={e => updateSettings({ ...settings, noiseStrengthMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.noiseStrengthMax}
                  onChange={e => updateSettings({ ...settings, noiseStrengthMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
          ...
          {forceTab === 'attraction' && (
            <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
            <h4 className="text-xs font-semibold text-blue-300 mb-2">🧲 Attraction</h4>
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.attractionStrengthMin}
                  onChange={e => updateSettings({ ...settings, attractionStrengthMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.attractionStrengthMax}
                  onChange={e => updateSettings({ ...settings, attractionStrengthMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <label className="block">
                <span className="text-xs text-slate-300">Point X</span>
                <input
                  type="number"
                  value={settings.attractionPoint.x}
                  onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, x: Number(e.target.value) } })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Point Y</span>
                <input
                  type="number"
                  value={settings.attractionPoint.y}
                  onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, y: Number(e.target.value) } })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
          ...
          {forceTab === 'vortex' && (
            <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
            <h4 className="text-xs font-semibold text-pink-300 mb-2">🌀 Vortex</h4>
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.vortexStrengthMin}
                  onChange={e => updateSettings({ ...settings, vortexStrengthMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.vortexStrengthMax}
                  onChange={e => updateSettings({ ...settings, vortexStrengthMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
            ...
            <label className="mt-2 inline-flex items-center gap-2 text-xs text-slate-200">
              <input
                type="checkbox"
                checked={showVortexForce}
                onChange={e => setShowVortexForce(e.target.checked)}
                className="accent-indigo-500"
              />
              Show vortex gizmo in viewport
            </label>
            </div>
          )}
          ...
          {forceTab === 'spin' && (
            ...
            <div className="grid grid-cols-2 gap-2 mb-2">
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.angularVelocityMin}
                  onChange={e => updateSettings({ ...settings, angularVelocityMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                ...
                <input
                  type="number"
                  value={settings.angularVelocityMax}
                  onChange={e => updateSettings({ ...settings, angularVelocityMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 mt-2">
              <label className="block">
                <span className="text-xs text-slate-300">Random Rotation Min (deg)</span>
                <input
                  type="number"
                  min="-360"
                  max="360"
                  value={settings.randomRotationMin}
                  onChange={e => updateSettings({ ...settings, randomRotationMin: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Random Rotation Max (deg)</span>
                <input
                  type="number"
                  min="-360"
                  max="360"
                  value={settings.randomRotationMax}
                  onChange={e => updateSettings({ ...settings, randomRotationMax: Number(e.target.value) })}
                  className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                />
              </label>
            </div>
        ...
        <div className="space-y-2">
          <div className="flex flex-wrap gap-2">
            {[
              ...
              <button
                key={tab.id}
                onClick={() => setCurveTab(tab.id as any)}
                className={`px-2 py-1 rounded text-[11px] border ${curveTab === tab.id ? 'bg-purple-600 border-purple-400' : 'bg-slate-800 border-slate-700 hover:bg-slate-700'}`}
              >
                {tab.label}
              </button>
            ))}
          ...
          {curveTab === 'size' && (
            <>
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  <span className="text-xs text-slate-300">Size Min</span>
                  <input
                    type="number"
                    step="0.05"
                    value={settings.sizeMin}
                    onChange={e => updateSettings({ ...settings, sizeMin: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
                <label className="block">
                  <span className="text-xs text-slate-300">Size Max</span>
                  <input
                    type="number"
                    step="0.05"
                    value={settings.sizeMax}
                    onChange={e => updateSettings({ ...settings, sizeMax: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  ...
                  <input
                    type="number"
                    step="0.05"
                    value={settings.speedMultiplierMin}
                    onChange={e => updateSettings({ ...settings, speedMultiplierMin: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    type="number"
                    step="0.05"
                    value={settings.speedMultiplierMax}
                    onChange={e => updateSettings({ ...settings, speedMultiplierMax: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
              </div>
          ...
          {curveTab === 'weight' && (
            <>
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  <span className="text-xs text-slate-300">Weight Min</span>
                  <input
                    type="number"
                    step="0.05"
                    value={settings.weightMin}
                    onChange={e => updateSettings({ ...settings, weightMin: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
                <label className="block">
                  <span className="text-xs text-slate-300">Weight Max</span>
                  <input
                    type="number"
                    step="0.05"
                    value={settings.weightMax}
                    onChange={e => updateSettings({ ...settings, weightMax: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
              </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-700">
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  ...
                  <input
                    type="number"
                    value={settings.spinMin}
                    onChange={e => updateSettings({ ...settings, spinMin: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
                <label className="block">
                  ...
                  <input
                    type="number"
                    value={settings.spinMax}
                    onChange={e => updateSettings({ ...settings, spinMax: Number(e.target.value) })}
                    className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                  />
                </label>
              </div>
    ...
    export: (
      <CollapsibleSection title="💾 Export Settings" isOpen={exportOpen} onToggle={() => setExportOpen(!exportOpen)}>
        <div className="space-y-2">
            ...
            {settings.emitter.looping && (
              <>
                ...
                {settings.emitter.prewarm && (
                  <div className="text-blue-400">⚡ Prewarm: ON</div>
                )}
              </>
            )}
          </div>
          ...
          <label className="block">
            <span className="text-xs text-slate-300">Frame Size (px)</span>
            <input type="number" min="128" max="1024" step="64" value={settings.frameSize} onChange={e => updateSettings({ ...settings, frameSize: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
          </label>

          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="text-xs text-slate-300">FPS</span>
              <input type="number" min="1" max="120" value={settings.fps} onChange={e => updateSettings({ ...settings, fps: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Duration (sec)</span>
              <input type="number" min="0.1" max="10" step="0.1" value={settings.duration} onChange={e => updateSettings({ ...settings, duration: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
          </div>

          <div className="space-y-1">
            <div className="text-xs text-slate-300">Export Channels</div>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 text-xs">
                <input type="checkbox" checked={settings.exportSettings.exportTranslate} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportTranslate: e.target.checked } })} />
                Translate
              </label>
              <label className="flex items-center gap-2 text-xs">
                <input type="checkbox" checked={settings.exportSettings.exportRotate} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportRotate: e.target.checked } })} />
                Rotate
              </label>
              <label className="flex items-center gap-2 text-xs">
                <input type="checkbox" checked={settings.exportSettings.exportScale} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportScale: e.target.checked } })} />
                Scale
              </label>
              <label className="flex items-center gap-2 text-xs">
                <input type="checkbox" checked={settings.exportSettings.exportColor} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportColor: e.target.checked } })} />
                Color
              </label>
            </div>
          </div>
