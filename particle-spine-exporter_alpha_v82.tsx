/**
 * Particle → Spine Exporter v82 (Alpha)
 *
 * Version: 82
 * Date: 2025-11-23
 *
 * Major Changes in v82:
 * - Fixed prewarm loop export: prewarm particle transforms now correctly saved at animation start
 * - Improved attachment visibility logic: attachments hidden when Scale < 0.05 OR alpha < 1
 * - Attachments re-enabled when Scale grows above 0.05 or alpha grows above 1
 *
  ...
  for (const particleId of particleIds) {
    ...
    for (let frameIdx = 0; frameIdx < frames.length; frameIdx++) {
      ...
      if (particle && isVisible) {
        ...
        if (settings.exportSettings.exportRotate && (isFirstFrame || isLastFrame || visibilityChanged || prevRotation === null || shouldCreateKey(prevRotation, normalizedAngle, ROTATION_THRESHOLD))) {
    ...
    if (hasAppeared) {
      ...
      if (Object.keys(boneAnimation).length > 0) {
        animations.particle_anim.bones[boneName] = boneAnimation;
      }
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v82
          </h1>
          <p className="text-xs text-slate-400">Major Update: Prewarm Loop Export Fixed • Improved Attachment Visibility • Better Timeline Handling</p>
        </header>
            ...
            <CollapsibleSection title="💾 Export Settings" isOpen={exportOpen} onToggle={() => setExportOpen(!exportOpen)}>
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div>
                    <label className="text-xs text-slate-400">Color Threshold (RGB)</label>
                    <input
                      type="number"
