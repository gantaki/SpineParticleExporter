 ...
 * - Reworked editor navigation with Initial/Update tabs instead of split curve/force panels
 * - Added randomized start properties (speed, mass, color, size, rotation/spin) with non-uniform options
 * - Defaulted vortex forces to zero and refreshed emitter/export layout readability
*
...
class ParticleSystem {
  ...
  update(dt: number, skipTimeReset: boolean = false) {
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      const speedMultiplier = lerp(
        this.settings.speedMultiplierMin,
        this.settings.speedMultiplierMax,
        clamp01(evaluateCurve(this.settings.speedOverLifetime, t))
      );
      ...
      const angularVelocity = lerp(
        this.settings.angularVelocityMin,
        this.settings.angularVelocityMax,
        clamp01(evaluateCurve(this.settings.angularVelocityOverLifetime, t))
      );
  ...
  spawnParticle() {
    ...
    const baseScaleX = this.settings.nonUniformScale ? baseScale * this.settings.scaleRatioX : baseScale;
    const baseScaleY = this.settings.nonUniformScale ? baseScale * this.settings.scaleRatioY : baseScale;
...
const ParticleSpineExporter: React.FC = () => {
  ...
  const [forceTab, setForceTab] = useState<'gravity' | 'noise' | 'attraction' | 'vortex' | 'spin'>('gravity');
  const [curveTab, setCurveTab] = useState<'size' | 'speed' | 'weight' | 'attraction' | 'spin'>('size');
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs text-slate-300">Position X</span>
              <input type="number" value={settings.emitter.position.x} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, position: { ...settings.emitter.position, x: Number(e.target.value) }}})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Position Y</span>
              <input type="number" value={settings.emitter.position.y} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, position: { ...settings.emitter.position, y: Number(e.target.value) }}})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Emission Rate</span>
              <input type="number" min="1" max="500" value={settings.emitter.rate} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, rate: Number(e.target.value) }})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Max Particles</span>
              <input type="number" min="1" max="2000" value={settings.emitter.maxParticles} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, maxParticles: Number(e.target.value) }})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              <span className="text-xs text-slate-300">Start Delay (s)</span>
              <input type="number" min="0" max="5" step="0.1" value={settings.emitter.startDelay} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, startDelay: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <div className="grid grid-cols-2 gap-2">
              <label className="flex items-center gap-2 text-xs bg-slate-900/60 border border-slate-700 rounded px-2 py-1">
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-3">
            <label className="block">
              ...
              <select value={settings.emitter.emissionType} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, emissionType: e.target.value as any } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs">
            ...
            {settings.emitter.emissionType === 'burst' && (
              <div className="space-y-2 pl-2 border-l-2 border-purple-500">
                <label className="block">
                  ...
                  <input type="number" min="1" max="500" value={settings.emitter.burstCount} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, burstCount: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                </label>
                <label className="block">
                  <span className="text-xs text-slate-300">Burst Cycles</span>
                  <input type="number" min="1" max="20" value={settings.emitter.burstCycles} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, burstCycles: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                </label>
                <label className="block">
                  <span className="text-xs text-slate-300">Burst Interval (s)</span>
                  <input type="number" min="0.05" max="5" step="0.05" value={settings.emitter.burstInterval} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, burstInterval: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                </label>
              </div>
            )}
            ...
            {settings.emitter.emissionType === 'duration' && (
              <div className="space-y-2 pl-2 border-l-2 border-blue-500">
                <label className="block">
                  <span className="text-xs text-slate-300">Duration Start (s)</span>
                  <input type="number" min="0" max="10" step="0.1" value={settings.emitter.durationStart} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, durationStart: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                </label>
                <label className="block">
                  <span className="text-xs text-slate-300">Duration End (s)</span>
                  <input type="number" min="0" max="10" step="0.1" value={settings.emitter.durationEnd} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, durationEnd: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                </label>
              </div>
            )}
          </div>
      ...
      <CollapsibleSection title="✨ Particle Settings" isOpen={particleOpen} onToggle={() => setParticleOpen(!particleOpen)}>
        <div className="space-y-2">
          ...
          <label className="block">
            ...
            <select value={settings.particleSprite} onChange={e => { const newSprite = e.target.value as any; updateSettings({ ...settings, particleSprite: newSprite }); if (newSprite !== 'custom') { updateSpriteCanvas(newSprite, null); } }} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs">
              <option value="circle">⚪ Circle</option>
          ...
          {settings.particleSprite === 'custom' && (
            <button onClick={() => spriteInputRef.current?.click()} className="w-full px-2 py-1 bg-blue-600 hover:bg-blue-700 rounded text-xs">Upload Custom Sprite</button>
          )}
        </div>
      </CollapsibleSection>
    ),
      ...
      <CollapsibleSection title="🚀 Initial" isOpen={initialOpen} onToggle={() => setInitialOpen(!initialOpen)}>
        <div className="space-y-3">
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            ...
            <p className="text-[10px] text-slate-400">Initial launch speed is randomized between these bounds.</p>
          </div>

          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <label className="block">
                <span className="text-xs text-slate-300">Mass / Weight Min</span>
                <input type="number" step="0.05" value={settings.startWeightMin} onChange={e => updateSettings({ ...settings, startWeightMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Mass / Weight Max</span>
                <input type="number" step="0.05" value={settings.startWeightMax} onChange={e => updateSettings({ ...settings, startWeightMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
            </div>
                ...
                <div key={entry.key} className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input
                      ...
                      className="w-16 px-2 py-1 bg-slate-900 border border-slate-700 rounded text-xs"
                    />
                  </div>
                </div>
              ))}
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <label className="block">
                <span className="text-xs text-slate-300">Start Size Min</span>
                <input type="number" step="0.05" value={settings.startSizeMin} onChange={e => updateSettings({ ...settings, startSizeMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Start Size Max</span>
                <input type="number" step="0.05" value={settings.startSizeMax} onChange={e => updateSettings({ ...settings, startSizeMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
            </div>
            <div className="grid grid-cols-3 gap-2">
              <label className="flex items-center gap-2 text-xs col-span-3 sm:col-span-1">
                <input type="checkbox" checked={settings.nonUniformScale} onChange={e => updateSettings({ ...settings, nonUniformScale: e.target.checked })} className="accent-orange-400" />
                <span>Non-uniform</span>
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Scale X Ratio</span>
                <input type="number" step="0.05" value={settings.scaleRatioX} onChange={e => updateSettings({ ...settings, scaleRatioX: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Scale Y Ratio</span>
                <input type="number" step="0.05" value={settings.scaleRatioY} onChange={e => updateSettings({ ...settings, scaleRatioY: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
            </div>
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <label className="block">
                <span className="text-xs text-slate-300">Start Rotation Min (deg)</span>
                <input type="number" min="-360" max="360" value={settings.randomRotationMin} onChange={e => updateSettings({ ...settings, randomRotationMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Start Rotation Max (deg)</span>
                <input type="number" min="-360" max="360" value={settings.randomRotationMax} onChange={e => updateSettings({ ...settings, randomRotationMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Start Spin Min (deg/s)</span>
                <input type="number" min="-1080" max="1080" value={settings.randomSpinMin} onChange={e => updateSettings({ ...settings, randomSpinMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Start Spin Max (deg/s)</span>
                <input type="number" min="-1080" max="1080" value={settings.randomSpinMax} onChange={e => updateSettings({ ...settings, randomSpinMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
            </div>
      ...
      <CollapsibleSection title="🔄 Update" isOpen={updateOpen} onToggle={() => setUpdateOpen(!updateOpen)}>
        <div className="space-y-3">
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <label className="block">
                <span className="text-xs text-slate-300">Update Size Min</span>
                <input type="number" step="0.05" value={settings.sizeMin} onChange={e => updateSettings({ ...settings, sizeMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
              <label className="block">
                <span className="text-xs text-slate-300">Update Size Max</span>
                <input type="number" step="0.05" value={settings.sizeMax} onChange={e => updateSettings({ ...settings, sizeMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
              </label>
            </div>
            <CurveEditor
              label="Size"
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-3">
            <div className="flex flex-wrap gap-2">
              {[{ id: 'speed', label: 'Speed Mult' }, { id: 'weight', label: 'Weight' }, { id: 'attraction', label: 'Attraction' }, { id: 'spin', label: 'Spin' }].map(tab => (
            ...
            {curveTab === 'speed' && (
              <>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Speed Mult Min</span>
                    <input type="number" step="0.05" value={settings.speedMultiplierMin} onChange={e => updateSettings({ ...settings, speedMultiplierMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Speed Mult Max</span>
                    <input type="number" step="0.05" value={settings.speedMultiplierMax} onChange={e => updateSettings({ ...settings, speedMultiplierMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
                <CurveEditor label="Speed" curve={settings.speedOverLifetime} onChange={curve => updateSettings({ ...settings, speedOverLifetime: curve })} onReset={() => updateSettings({ ...settings, speedOverLifetime: DEFAULT_CURVE_PRESETS.speed })} min={0} max={1} />
              </>
            )}
            ...
            {curveTab === 'weight' && (
              <>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Weight Curve Min</span>
                    <input type="number" step="0.05" value={settings.weightMin} onChange={e => updateSettings({ ...settings, weightMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Weight Curve Max</span>
                    <input type="number" step="0.05" value={settings.weightMax} onChange={e => updateSettings({ ...settings, weightMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
            ...
            {curveTab === 'attraction' && (
              <>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Min</span>
                    <input type="number" step="0.05" value={settings.attractionStrengthMin} onChange={e => updateSettings({ ...settings, attractionStrengthMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Max</span>
                    <input type="number" step="0.05" value={settings.attractionStrengthMax} onChange={e => updateSettings({ ...settings, attractionStrengthMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
                ...
                <label className="block mt-1">
                  <span className="text-xs text-slate-300">Attraction Point</span>
                  <div className="grid grid-cols-2 gap-2 mt-1">
                    <input type="number" value={settings.attractionPoint.x} onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, x: Number(e.target.value) } })} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                    <input type="number" value={settings.attractionPoint.y} onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, y: Number(e.target.value) } })} className="w-full px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </div>
                </label>
              </>
            )}
            ...
            {curveTab === 'spin' && (
              <>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Spin Min (deg/s)</span>
                    <input type="number" step="1" value={settings.spinMin} onChange={e => updateSettings({ ...settings, spinMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Spin Max (deg/s)</span>
                    <input type="number" step="1" value={settings.spinMax} onChange={e => updateSettings({ ...settings, spinMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
                ...
                <CurveEditor label="Angular Velocity" curve={settings.angularVelocityOverLifetime} onChange={curve => updateSettings({ ...settings, angularVelocityOverLifetime: curve })} onReset={() => updateSettings({ ...settings, angularVelocityOverLifetime: DEFAULT_CURVE_PRESETS.angularVelocity })} min={0} max={1} />
              </>
            )}
          </div>
          ...
          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-3">
            <div className="flex flex-wrap gap-2">
              {[{ id: 'gravity', label: 'Gravity/Drag' }, { id: 'noise', label: 'Noise' }, { id: 'attraction', label: 'Attraction' }, { id: 'vortex', label: 'Vortex' }, { id: 'spin', label: 'Spin' }].map(tab => (
            ...
            {forceTab === 'gravity' && (
              <div className="bg-slate-900/30 p-2 rounded border border-slate-700 space-y-2">
                <h4 className="text-xs font-semibold text-green-300">🌍 Gravity & Drag</h4>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Gravity Min</span>
                    <input type="number" value={settings.gravityMin} onChange={e => updateSettings({ ...settings, gravityMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Gravity Max</span>
                    <input type="number" value={settings.gravityMax} onChange={e => updateSettings({ ...settings, gravityMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Drag Min</span>
                    <input type="number" step="0.01" value={settings.dragMin} onChange={e => updateSettings({ ...settings, dragMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Drag Max</span>
                    <input type="number" step="0.01" value={settings.dragMax} onChange={e => updateSettings({ ...settings, dragMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
            ...
            {forceTab === 'noise' && (
              <div className="bg-slate-900/30 p-2 rounded border border-slate-700 space-y-2">
                <h4 className="text-xs font-semibold text-amber-300">🌪️ Noise</h4>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Min</span>
                    <input type="number" value={settings.noiseStrengthMin} onChange={e => updateSettings({ ...settings, noiseStrengthMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Max</span>
                    <input type="number" value={settings.noiseStrengthMax} onChange={e => updateSettings({ ...settings, noiseStrengthMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    ...
                    <input type="number" step="0.001" value={settings.noiseFrequency} onChange={e => updateSettings({ ...settings, noiseFrequency: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Speed</span>
                    <input type="number" step="0.1" value={settings.noiseSpeed} onChange={e => updateSettings({ ...settings, noiseSpeed: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
              </div>
            )}
            ...
            {forceTab === 'attraction' && (
              <div className="bg-slate-900/30 p-2 rounded border border-slate-700 space-y-2">
                <h4 className="text-xs font-semibold text-blue-300">🧲 Attraction</h4>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Attraction Point X</span>
                    <input type="number" value={settings.attractionPoint.x} onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, x: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Attraction Point Y</span>
                    <input type="number" value={settings.attractionPoint.y} onChange={e => updateSettings({ ...settings, attractionPoint: { ...settings.attractionPoint, y: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Min</span>
                    <input type="number" value={settings.attractionStrengthMin} onChange={e => updateSettings({ ...settings, attractionStrengthMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Max</span>
                    <input type="number" value={settings.attractionStrengthMax} onChange={e => updateSettings({ ...settings, attractionStrengthMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
            ...
            {forceTab === 'vortex' && (
              <div className="bg-slate-900/30 p-2 rounded border border-slate-700 space-y-2">
                <h4 className="text-xs font-semibold text-pink-300">🌀 Vortex</h4>
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    <span className="text-xs text-slate-300">Center X</span>
                    <input type="number" value={settings.vortexPoint.x} onChange={e => updateSettings({ ...settings, vortexPoint: { ...settings.vortexPoint, x: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Center Y</span>
                    <input type="number" value={settings.vortexPoint.y} onChange={e => updateSettings({ ...settings, vortexPoint: { ...settings.vortexPoint, y: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Min</span>
                    <input type="number" value={settings.vortexStrengthMin} onChange={e => updateSettings({ ...settings, vortexStrengthMin: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                  <label className="block">
                    <span className="text-xs text-slate-300">Strength Max</span>
                    <input type="number" value={settings.vortexStrengthMax} onChange={e => updateSettings({ ...settings, vortexStrengthMax: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
                  </label>
                </div>
            ...
            {forceTab === 'spin' && (
              <div className="bg-slate-900/30 p-2 rounded border border-slate-700 space-y-2">
                <h4 className="text-xs font-semibold text-emerald-300">🌀 Rotation</h4>
                <CurveEditor label="Angular Velocity" curve={settings.angularVelocityOverLifetime} onChange={curve => updateSettings({ ...settings, angularVelocityOverLifetime: curve })} onReset={() => updateSettings({ ...settings, angularVelocityOverLifetime: DEFAULT_CURVE_PRESETS.angularVelocity })} min={0} max={1} />
                <p className="text-[10px] text-slate-500">Continuous rotation over particle lifetime.</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-3 gap-2">
            <label className="block">
              ...
              <input type="number" min="0.1" max="10" step="0.1" value={settings.duration} onChange={e => updateSettings({ ...settings, duration: Number(e.target.value), exportSettings: { ...settings.exportSettings } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">FPS</span>
              <input type="number" min="8" max="120" step="1" value={settings.fps} onChange={e => updateSettings({ ...settings, fps: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Frame Size</span>
              <input type="number" min="128" max="1024" step="32" value={settings.frameSize} onChange={e => updateSettings({ ...settings, frameSize: Number(e.target.value) })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
          </div>

          <div className="bg-slate-900/40 p-3 rounded border border-slate-700 space-y-2">
            <span className="text-xs text-slate-300">Export Channels</span>
            <div className="grid grid-cols-4 gap-2">
              <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.exportSettings.exportTranslate} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportTranslate: e.target.checked } })} /> Translate</label>
              <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.exportSettings.exportRotate} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportRotate: e.target.checked } })} /> Rotate</label>
              <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.exportSettings.exportScale} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportScale: e.target.checked } })} /> Scale</label>
              <label className="flex items-center gap-2 text-xs"><input type="checkbox" checked={settings.exportSettings.exportColor} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, exportColor: e.target.checked } })} /> Color</label>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="text-xs text-slate-300">Position Threshold</span>
              <input type="number" step="0.1" min="0" value={settings.exportSettings.positionThreshold} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, positionThreshold: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Rotation Threshold</span>
              <input type="number" step="0.1" min="0" value={settings.exportSettings.rotationThreshold} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, rotationThreshold: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Scale Threshold</span>
              <input type="number" step="0.01" min="0" value={settings.exportSettings.scaleThreshold} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, scaleThreshold: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Color Threshold (RGBA Sum)</span>
              <input type="number" step="1" min="0" max="1020" value={settings.exportSettings.colorThreshold} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, colorThreshold: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
          </div>

          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              <span className="text-xs text-slate-300">Translate Direction (deg)</span>
              <input type="number" step="1" min="0" max="180" value={settings.exportSettings.translateDirectionAngle} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, translateDirectionAngle: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
            <label className="block">
              <span className="text-xs text-slate-300">Rotation Direction Threshold</span>
              <input type="number" step="0.1" min="0" max="180" value={settings.exportSettings.rotationDirectionThreshold} onChange={e => updateSettings({ ...settings, exportSettings: { ...settings.exportSettings, rotationDirectionThreshold: Number(e.target.value) } })} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs" />
            </label>
          </div>
        </div>
      </CollapsibleSection>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 text-white p-4">
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          ...
          <p className="text-xs text-slate-400">Spine 4.2.00 JSON • Prewarm animation included</p>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr_1fr_1fr] gap-4">
          <div className="space-y-3">
            <div className="bg-slate-800/50 backdrop-blur rounded-lg p-3 border border-slate-700">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={() => setShowGrid(!showGrid)} className={`px-2 py-1 rounded text-xs ${showGrid ? 'bg-cyan-600 hover:bg-cyan-700' : 'bg-slate-700 hover:bg-slate-600'}`}>
                    #
                  </button>
