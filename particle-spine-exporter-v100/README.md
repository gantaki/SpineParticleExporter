# Particle Spine Exporter v100 - Modular Structure

Modularized source for the v100 release (burst/duration export fixes, lowered thresholds, per-emitter sprites). Edit these files during development and rebuild the standalone TSX when ready to distribute.

## 📁 File Structure
```
particle-spine-exporter-v100/
...
- **Build standalone**: Run `npm run build:v100` to produce `particle-spine-exporter_alpha_v100.tsx` from the modules.
- **Standalone testing**: Open `standalone.html` (pointing at the v100 file) or serve via `npm run serve`.

## 🔧 Available Commands
```bash
# Build standalone v100 from modules
npm run build:v100
...
2. Rebuild the standalone TSX with `npm run build:v100`.
3. Commit the regenerated `particle-spine-exporter_alpha_v100.tsx` (or bump version per CLAUDE.md guidance).
