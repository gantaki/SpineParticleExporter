/**
 * Particle → Spine Exporter v84d (Alpha)
*
 * Version: 84d
 * Date: 2025-11-23
*
 * Major Changes in v84d:
 * - Restored stable Spine metadata and added dual Color/RGBA slot exports
 * - Expanded viewport visuals with grid and vortex overlays plus stronger noise
 * - UI polish: full-width editors, reset for color over lifetime, draggable columns
 *
...
const DEFAULT_CURVE_PRESETS: { [key: string]: Curve } = {
  size: { points: [{ time: 0, value: 1.0 }, { time: 1, value: 0.2 }], interpolation: 'linear' },
...
class ParticleSystem {
  ...
  render(ctx: CanvasRenderingContext2D, showEmitter: boolean, showGrid: boolean, showVortexForce: boolean, zoom: number, spriteCanvas: HTMLCanvasElement | null, backgroundImage?: HTMLImageElement | null, bgPosition?: { x: number; y: number }) {
    ...
    if (showVortexForce) {
      const vortexStrength = evaluateCurve(this.settings.vortexStrengthOverLifetime, 0.5);
...
const ParticleSpineExporter: React.FC = () => {
  ...
  const [showVortexForce, setShowVortexForce] = useState(true);
  ...
  const [panelColumns, setPanelColumns] = useState<{ left: string[]; right: string[] }>({ left: [...PANEL_IDS], right: [] });
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <label className="block">
              ...
              <select value={settings.emitter.prewarm ? 'yes' : 'no'} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, prewarm: e.target.value === 'yes' }})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs">
          ...
          <div className="grid grid-cols-2 gap-2">
            <label className="block">
              ...
              <select value={settings.emitter.looping ? 'yes' : 'no'} onChange={e => updateSettings({ ...settings, emitter: { ...settings.emitter, looping: e.target.value === 'yes' }})} className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs">
  ...
  const handlePanelDrop = (column: 'left' | 'right') => {
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v84d
          </h1>
          <p className="text-xs text-slate-400">Spine 4.2.00 JSON • Prewarm animation included</p>
        </header>

        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr_1fr] gap-4">
          <div className="space-y-3">
            <div className="bg-slate-800/50 backdrop-blur rounded-lg p-3 border border-slate-700">
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-1.5">
                  ...
                  <button onClick={() => setShowVortexForce(!showVortexForce)} className={`px-2 py-1 rounded text-xs ${showVortexForce ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-slate-700 hover:bg-slate-600'}`}>
                    🌀
                  </button>
