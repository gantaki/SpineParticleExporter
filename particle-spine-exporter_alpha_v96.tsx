/**
 * Particle → Spine Exporter v96 (Alpha)
 *
 * Version: 96
 * Date: 2025-11-25
 *
 * Major Changes in v96:
 * - When Loop+Prewarm enabled, animations named "loop" and "prewarm" (not "particle_anim")
 * - Added duplicate frame 0 to end of loop animation for seamless cycling
 * - Prewarm animation now only includes bones that have offset data from loop
 * - Prewarm duration adjusted to match longest bone animation
 * - Updated UI version display to v96
 */
...
function generateSpineJSON(frames: BakedFrame[], prewarmFrames: BakedFrame[], settings: ParticleSettings): string {
  ...
  // This ensures we only create bones for particles that appear in the main loop
  collectParticleIds(frames);

  let particleIds = Array.from(allParticleIds).sort((a, b) => a - b);

  // If both looping and prewarm are enabled, limit bones to rate × duration
  // This ensures we only export bones from ID 0 to (rate × duration - 1)
  if (settings.emitter.looping && settings.emitter.prewarm) {
    const maxBoneCount = Math.floor(settings.emitter.rate * settings.duration);
    particleIds = particleIds.filter(id => id < maxBoneCount);
  }
