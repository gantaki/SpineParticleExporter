...
const DEFAULT_SETTINGS: ParticleSettings = {
  ...
  colorOverLifetime: {
    points: [
      { time: 0, color: { r: 255, g: 200, b: 100, a: 255 } },
      { time: 1, color: { r: 255, g: 50, b: 50, a: 255 } }
    ]
  },
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex-1 text-xs font-mono text-slate-400">
          RGB({color.r}, {color.g}, {color.b}) A={color.a}
        </div>
      </div>
    </div>
  );
...
}> = ({ currentTime, duration, isPlaying, playbackSpeed, onTimeChange, onPlayPause, onSpeedChange }) => {
  const [isDragging, setIsDragging] = useState(false);
...
function bakeParticleAnimation(settings: ParticleSettings): BakedFrame[] {
  ...
  // For looping animations, simulate longer to ensure seamless loop
  let totalSimTime = settings.duration;
  if (settings.emitter.looping) {
    // Simulate extra time equal to max particle lifetime
    totalSimTime = settings.duration + settings.lifeTimeMax;
  }
  ...
  for (let i = 0; i < totalFrameCount; i++) {
    ...
    // Only save frames within the actual duration
    if (i < frameCount) {
...
function generateSpineJSON(frames: BakedFrame[], settings: ParticleSettings): string {
  ...
  for (const particleId of particleIds) {
    ...
    const COLOR_THRESHOLD = 10; // RGB change threshold
    ...
    for (let frameIdx = 0; frameIdx < frames.length; frameIdx++) {
      ...
      if (particle && isVisible) {
        ...
        const colorChanged = prevColor === null ||
          Math.abs((currentColor.r - prevColor.r) * 255) > COLOR_THRESHOLD ||
          Math.abs((currentColor.g - prevColor.g) * 255) > COLOR_THRESHOLD ||
          Math.abs((currentColor.b - prevColor.b) * 255) > COLOR_THRESHOLD ||
          Math.abs(currentColor.a - prevColor.a) > 0.05;
    ...
    if (hasAppeared && translateKeys.length > 0) {
      ...
      if (Object.keys(slotAnimation).length > 0) {
        animations.particle_anim.slots[slotName] = slotAnimation;
      }
    }
  }
...
const ParticleSpineExporter: React.FC = () => {
  ...
  const simulateToTime = useCallback((targetTime: number) => {
    if (!systemRef.current) return;
    
    // Reset and simulate up to target time
    systemRef.current.reset();
    const dt = 1 / 60; // 60 FPS simulation
    let simTime = 0;
    
    while (simTime < targetTime) {
      const step = Math.min(dt, targetTime - simTime);
      systemRef.current.update(step);
      simTime += step;
    }
  ...
  }, [showEmitter, zoom, spriteCanvas, backgroundImage, bgPosition]);

  const handleTimelineTimeChange = useCallback((newTime: number) => {
    setCurrentTime(newTime);
    setIsPlaying(false);
    simulateToTime(newTime);
  }, [simulateToTime]);
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v79
          </h1>
          <p className="text-xs text-slate-400">New: Timeline with Scrubbing • Random Rotation • Improved Loop Export</p>
        </header>
