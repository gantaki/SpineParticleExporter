...
const DEFAULT_SETTINGS: ParticleSettings = {
  ...
  colorOverLifetime: {
    points: [
      ...
      { time: 1, color: { r: 255, g: 50, b: 50, a: 0 } }
    ]
  },
...
class ParticleSystem {
  ...
  render(ctx: CanvasRenderingContext2D, showEmitter: boolean, zoom: number, spriteCanvas: HTMLCanvasElement | null, backgroundImage?: HTMLImageElement | null, bgPosition?: { x: number; y: number }) {
    ...
    const centerX = this.settings.frameSize / 2;
    const centerY = this.settings.frameSize / 2;
    ...
    ctx.clearRect(-centerX / zoom, -centerY / zoom, this.settings.frameSize / zoom, this.settings.frameSize / zoom);
    ...
    for (const p of this.particles) {
      ...
      if (spriteCanvas) {
        ...
        ctx.drawImage(spriteCanvas, -size, -size, size * 2, size * 2);
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v75
          </h1>
          <p className="text-xs text-slate-400">Color Gradient • Zoom Control • Sprite Selection • Curve Resets</p>
        </header>
