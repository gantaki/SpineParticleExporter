/**
 * Particle → Spine Exporter v89 (Alpha)
 *
 * Version: 89
 * Date: 2026-06-10
 *
 * Major Changes in v89:
...
const cloneGradient = (gradient: ColorGradient): ColorGradient => ({
...
const DEFAULT_COLOR_GRADIENT: ColorGradient = {
  points: [
...
const DEFAULT_SETTINGS: ParticleSettings = {
  ...
  gravityOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.gravity),
  gravityRange: { min: 0, max: 0 },
  dragOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.drag),
  dragRange: { min: 0.98, max: 0.98 },

  sizeXOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.sizeX),
  sizeXRange: { min: 1, max: 1 },
  sizeYOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.sizeY),
  sizeYRange: { min: 1, max: 1 },
  speedOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.speed),
  speedRange: { min: 1, max: 1 },
  weightOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.weight),
  weightRange: { min: 1, max: 1 },
  spinOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.spin),
  spinRange: { min: 0, max: 0 },
  attractionOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.attraction),
  attractionRange: { min: 0, max: 0 },

  noiseStrengthOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.noise),
  noiseStrengthRange: { min: 0, max: 0 },
  noiseFrequencyRange: { min: 0.2, max: 0.6 },
  noiseSpeedRange: { min: 2.5, max: 6.0 },

  angularVelocityOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.angularVelocity),
  angularVelocityRange: { min: 0, max: 0 },

  vortexStrengthOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.vortex),
  ...
  colorOverLifetime: cloneGradient(DEFAULT_COLOR_GRADIENT),
...
const createDefaultSettings = (): ParticleSettings => JSON.parse(JSON.stringify(DEFAULT_SETTINGS));
...
function noise2D(x: number, y: number, time: number): Vec2 {
  const fastJitter = (simpleNoise(x * 2 + time * 6, y * 2 - time * 4) * 2 - 1);
  const driftAngle = simpleNoise(x * 0.35 + 50, y * 0.35 + time * 0.5) * Math.PI * 2;
  const wobble = Math.sin((x + y) * 0.5 + time * 8) * 0.5;
  const burst = (simpleNoise(x * 1.3 - time * 3, y * 1.3 + time * 2) * 2 - 1) * 0.6;

  const strength = Math.max(-1, Math.min(1, (fastJitter * 0.6 + wobble * 0.25 + burst * 0.4)));
  const angle = driftAngle + fastJitter * 0.75;
...
const ColorGradientEditor: React.FC<{
  ...
  useEffect(() => {
    const measure = () => {
      if (containerRef.current) {
        setWidth(Math.max(260, containerRef.current.clientWidth));
      }
    };
  ...
  useEffect(() => {
    if (selectedPoint !== null && selectedPoint >= gradient.points.length) {
      setSelectedPoint(gradient.points.length ? gradient.points.length - 1 : null);
      setShowColorPicker(gradient.points.length > 0);
    }
  }, [gradient.points.length, selectedPoint]);
...
const CurveEditor: React.FC<{
  ...
  min?: number;
  max?: number;
}> = ({ label, curve, onChange, onReset, min = -1, max = 1 }) => {
  const [selectedPoint, setSelectedPoint] = useState<number | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const svgRef = useRef<SVGSVGElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    if (selectedPoint !== null && selectedPoint >= curve.points.length) {
      setSelectedPoint(curve.points.length ? Math.max(0, curve.points.length - 1) : null);
    }
  }, [curve.points.length, selectedPoint]);
  ...
  const [displayMin, displayMax] = useMemo(() => {
    ...
    const rawMin = Math.min(baseMin, ...values);
    const rawMax = Math.max(baseMax, ...values);
    const span = Math.max(0.001, rawMax - rawMin);
    const paddingAmount = span * 0.05;
    return [rawMin - paddingAmount, rawMax + paddingAmount];
  }, [curve.points, min, max]);

  const valueToY = (value: number) => {
    const normalized = (value - displayMin) / (displayMax - displayMin);
...
class ParticleSystem {
  ...
  update(dt: number, skipTimeReset: boolean = false) {
    ...
    for (let i = this.particles.length - 1; i >= 0; i--) {
      ...
      const gravity = p.baseGravity * evaluateCurve(this.settings.gravityOverLifetime, t);
      const weight = p.baseWeight * weightMultiplier;
      p.vy += gravity * weight * dt;

      const noiseStrength = p.baseNoiseStrength * evaluateCurve(this.settings.noiseStrengthOverLifetime, t);
      if (noiseStrength !== 0) {
      ...
      const attractionStrength = p.baseAttraction * evaluateCurve(this.settings.attractionOverLifetime, t);
      ...
      const vortexStrength = p.baseVortexStrength * evaluateCurve(this.settings.vortexStrengthOverLifetime, t);
      if (vortexStrength !== 0) {
      ...
      const drag = p.baseDrag * evaluateCurve(this.settings.dragOverLifetime, t);
      p.vx *= drag;
      p.vy *= drag;
      ...
      const spinSpeed = p.baseSpinRate * evaluateCurve(this.settings.spinOverLifetime, t);
      p.rotation += spinSpeed * dt;

      const angularVelocity = p.baseAngularVelocity * evaluateCurve(this.settings.angularVelocityOverLifetime, t);
  ...
  spawnParticle() {
    ...
    const speed = this.settings.speedMin + Math.random() * (this.settings.speedMax - this.settings.speedMin);
...
const ParticleSpineExporter: React.FC = () => {
  const [settings, setSettings] = useState<ParticleSettings>(() => createDefaultSettings());
      ...
      <div className="max-w-6xl mx-auto">
        <header className="mb-4">
          <h1 className="text-2xl font-bold bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">
            Particle → Spine Exporter v89
          </h1>
          <p className="text-xs text-slate-400">Adaptive multiplier graphs, sharper noise motion, and streamlined spin & speed controls</p>
        </header>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => updateSettings({ ...settings, speedMin: Number(e.target.value) })}
                      className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                    />
                  </label>
                  <label className="block">
                    ...
                    <input
                      ...
                      onChange={e => updateSettings({ ...settings, speedMax: Number(e.target.value) })}
                      className="w-full mt-1 px-2 py-1 bg-slate-900 border border-slate-600 rounded text-xs"
                    />
                  </label>
                </div>
              ...
              <div className="space-y-3">
                <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
                  ...
                  <CurveEditor
                    ...
                    onReset={() => updateSettings({ ...settings, gravityOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.gravity) })}
                    max={1}
                  />
                  ...
                  <div className="mt-2">
                    <CurveEditor
                      ...
                      onReset={() => updateSettings({ ...settings, dragOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.drag) })}
                      max={1}
                    />
                  </div>
                </div>
                ...
                <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
                  ...
                  <CurveEditor
                    ...
                    onReset={() => updateSettings({ ...settings, noiseStrengthOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.noise) })}
                    max={1}
                  />
                ...
                <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
                  ...
                  <CurveEditor
                    ...
                    onReset={() => updateSettings({ ...settings, vortexStrengthOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.vortex) })}
                    max={1}
                  />
                ...
                <div className="bg-slate-900/30 p-2 rounded border border-slate-700">
                  ...
                  <CurveEditor
                    ...
                    onReset={() => updateSettings({ ...settings, angularVelocityOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.angularVelocity) })}
                    max={1}
                  />
            ...
            <CollapsibleSection title="📈 Curves Over Lifetime" isOpen={curvesOpen} onToggle={() => setCurvesOpen(!curvesOpen)}>
              <div className="space-y-2">
                ...
                <CurveEditor
                  ...
                  onReset={() => updateSettings({ ...settings, sizeXOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.sizeX) })}
                  max={1}
                />
                <RangeInput
                ...
                <CurveEditor
                  ...
                  onReset={() => updateSettings({ ...settings, sizeYOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.sizeY) })}
                  max={1}
                />
                <RangeInput
                ...
                <CurveEditor
                  ...
                  onReset={() => updateSettings({ ...settings, speedOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.speed) })}
                  max={1}
                />
                <RangeInput
                ...
                <CurveEditor
                  ...
                  onReset={() => updateSettings({ ...settings, weightOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.weight) })}
                  max={1}
                />
                <RangeInput
                ...
                <CurveEditor
                  ...
                  onReset={() => updateSettings({ ...settings, attractionOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.attraction) })}
                  max={1}
                />
                ...
                <div className="space-y-2 pt-2 border-t border-slate-700">
                  ...
                  <CurveEditor
                    ...
                    onReset={() => updateSettings({ ...settings, spinOverLifetime: cloneCurve(DEFAULT_CURVE_PRESETS.spin) })}
                    max={1}
                  />
                </div>
              </div>
